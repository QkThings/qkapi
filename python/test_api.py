...
def handleEventJson(api):
	...
	print "-----------------------------------------------"
	print " %-8s conn_id=%d address=%04x %s" % ("EVENT", connID, addr, time.strftime("%H:%M:%S"))
...
def handleDebugJson(api):
	...
	print "-----------------------------------------------"
	print " %-8s conn_id=%d address=%04x %s" % ("DEBUG", connID, addr, time.strftime("%H:%M:%S"))
...
if __name__ == "__main__":
	...
	api.get("/qk/conns/0/nodes/0", show_result=True)
	api.get("/qk/conns/0/nodes/0/device/sampling", show_result=True)
	api.set("/qk/conns/0/nodes/0/device/sampling/mode", "continuous", show_result=True)
	api.set("/qk/conns/0/nodes/0/device/sampling/frequency", 10, show_result=True)
	api.set("/qk/conns/0/nodes/0/device/cmds/update", show_result=True)
	api.get("/qk/conns/0/nodes/0/device/sampling", show_result=True)
	api.set("/qk/conns/0/nodes/123/device/sampling/frequency", 1, show_result=True)
	api.set("/qk/conns/0/nodes/0/device/sampling/frequency", "asd", show_result=True)
